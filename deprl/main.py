...
from deprl.utils import load_config_and_paths
...
def train(
    ...
    _test_environment = tonic_conf['test_environment'] if 'test_environment' in tonic_conf and not (tonic_conf['test_environment'] is None) else _environment
    test_env_args = config['test_env_args'] if 'test_env_args' in config else config['env_args']
    ...
    # Train.
    try:
        scores = trainer.run(config, **time_dict)
